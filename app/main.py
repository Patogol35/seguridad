...
async def scan_website(data: ScanRequest):
    try:
        ...
        if "x-frame-options" not in headers:
            ...
            recommendations.append("Agregar X-Frame-Options para prevenir clickjacking")

        if "content-security-policy" not in headers:
            issues.append("Missing Content-Security-Policy header")
            recommendations.append("Implementar Content-Security-Policy para mitigar XSS")

        if "strict-transport-security" not in headers:
            issues.append("Missing HSTS header")
            recommendations.append("Habilitar HSTS para forzar HTTPS")
        ...
        if XSS_PAYLOAD in xss_resp.text:
            ...
            recommendations.append("Escapar y sanitizar entradas de usuario")
        ...
        if any(e in sqli_resp.text.lower() for e in errors):
            ...
            recommendations.append("Usar queries parametrizadas / ORM")
